...
OUT="reports/b9_topk"; mkdir -p "${OUT}"
python scripts/generate_b9_topk_candidates.py --config "${ROOT}/run_config.json" --checkpoint "${CKPT}" \
  ...
  --out "${OUT}/candidates_k8.jsonl" --k 8 --seed 42000 --temperature 0.8 --top-p 0.9
python scripts/evaluate_strategy_multilabel_llm.py --model-path "${MODEL_PATH}" \
  --jsonl "${OUT}/candidates_k8.jsonl" --out "${OUT}/candidates_k8_multilabel.json" --blind-profile
