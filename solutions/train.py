...
def main():
    ...
    for epoch in range(cfg.num_epochs):
        ...
        running_param_orth_n_qk = 0.0
        running_param_orth_n_vo = 0.0
        running_param_orth_skipped_qk = 0.0
        running_param_orth_skipped_vo = 0.0
        running_param_orth_skipped_qk_shape = 0.0
        running_param_orth_skipped_qk_gqa = 0.0
        running_param_orth_skipped_vo_shape = 0.0
        running_param_orth_skipped_vo_gqa = 0.0
        running_param_orth_num_lora_layers = 0.0
        running_param_orth_num_base_layers = 0.0
        param_orth_calls = 0
        running_cls = 0.0
        ...
        for batch in pbar:
            ...
            if "param_orth_num_lora_layers" in losses:
                param_orth_calls += 1
                running_param_orth_n_qk += losses["param_orth_n_qk"].item()
                running_param_orth_n_vo += losses["param_orth_n_vo"].item()
                running_param_orth_skipped_qk += losses["param_orth_skipped_qk"].item()
                running_param_orth_skipped_vo += losses["param_orth_skipped_vo"].item()
                running_param_orth_skipped_qk_shape += losses.get("param_orth_skipped_qk_shape", torch.zeros(())).item()
                running_param_orth_skipped_qk_gqa += losses.get("param_orth_skipped_qk_gqa", torch.zeros(())).item()
                running_param_orth_skipped_vo_shape += losses.get("param_orth_skipped_vo_shape", torch.zeros(())).item()
                running_param_orth_skipped_vo_gqa += losses.get("param_orth_skipped_vo_gqa", torch.zeros(())).item()
                running_param_orth_num_lora_layers += losses["param_orth_num_lora_layers"].item()
                running_param_orth_num_base_layers += losses["param_orth_num_base_layers"].item()
            ...
            if "param_orth_n_qk" in losses:
                postfix["n_qk"] = f"{int(losses['param_orth_n_qk'].item())}"
                postfix["n_vo"] = f"{int(losses['param_orth_n_vo'].item())}"
        ...
        epoch_record: Dict[str, float] = {
            ...
            "param_orth_calls": param_orth_calls,
            "param_orth_avg_n_qk": running_param_orth_n_qk / max(1, param_orth_calls),
            "param_orth_avg_n_vo": running_param_orth_n_vo / max(1, param_orth_calls),
            "param_orth_avg_skipped_qk": running_param_orth_skipped_qk / max(1, param_orth_calls),
            "param_orth_avg_skipped_vo": running_param_orth_skipped_vo / max(1, param_orth_calls),
            "param_orth_avg_skipped_qk_shape": running_param_orth_skipped_qk_shape / max(1, param_orth_calls),
            "param_orth_avg_skipped_qk_gqa": running_param_orth_skipped_qk_gqa / max(1, param_orth_calls),
            "param_orth_avg_skipped_vo_shape": running_param_orth_skipped_vo_shape / max(1, param_orth_calls),
            "param_orth_avg_skipped_vo_gqa": running_param_orth_skipped_vo_gqa / max(1, param_orth_calls),
            "param_orth_avg_lora_layers": running_param_orth_num_lora_layers / max(1, param_orth_calls),
            "param_orth_avg_base_layers": running_param_orth_num_base_layers / max(1, param_orth_calls),
