...
try:
    from .losses_param_orth import param_orth_losses
    ...
    if not should_compute_orth:
        ...
        if _PARAM_ORTH_AVAILABLE and (cfg.lambda_param_orth_qk > 0 or cfg.lambda_param_orth_vo > 0):
            if global_step % cfg.param_orth_every_n_steps == 0:
                ...
                param_orth_stats = {
                    "param_orth_n_qk": po_losses.get("n_qk", zero).detach(),
                    "param_orth_n_vo": po_losses.get("n_vo", zero).detach(),
                    "param_orth_skipped_qk": po_losses.get("skipped_qk", zero).detach(),
                    "param_orth_skipped_vo": po_losses.get("skipped_vo", zero).detach(),
                    "param_orth_skipped_qk_shape": po_losses.get("skipped_qk_shape", zero).detach(),
                    "param_orth_skipped_qk_gqa": po_losses.get("skipped_qk_gqa", zero).detach(),
                    "param_orth_skipped_vo_shape": po_losses.get("skipped_vo_shape", zero).detach(),
                    "param_orth_skipped_vo_gqa": po_losses.get("skipped_vo_gqa", zero).detach(),
                    "param_orth_num_lora_layers": po_losses.get("num_lora_layers", zero).detach(),
                    "param_orth_num_base_layers": po_losses.get("num_base_layers", zero).detach(),
                }
